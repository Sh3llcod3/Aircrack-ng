...
class functions:
    ...
    def getAvailableWifiCardsAlt(choice):
        ...
        def getCardNameAlt():
            ...
            questionArray = ["[+]Ok, Now which interface shall I host the AP on?  ~#  ","[+]Which interface is connected to the internet?  ~#  "]
            cardInterface = ""
            if choice == "1":
    ...
    """)
            ...
            userGpuMenuChoice = input("|MENU|POST_CRACK|GPU|DRIVER_DOWNLOAD|(Do you have an Nvidia GPU or AMD GPU? [a/n]) ~# ")
    ...
    """)
        if input("Download and setup hashcat for GPU-WPA cracking? [y/n] >> ").lower().startswith("y"):
            ...
            bashRun("\necho '\n[+] Hashcat is now ready to use, it will be inside the %s folder ' " %(hashcatDownloadPathTk))
            os._exit(1)
        else:
            mainMenu()
    ...
    def depandancies(): 
        ...
        if bashReturnValue("/usr/bin/env ping -c1 8.8.8.8 >/dev/null 2>/dev/null") == '0':
            subprocess.call("echo '[-] 25% done';sudo apt update --allow-unauthenticated 2>/dev/null && echo '[-] 50% done' && sudo apt install xterm -y --allow-unauthenticated >/dev/null 2>/dev/null",shell=True)
...
class main:
    ...
    def EvilTwinFakeAP():
        ...
        try:
            ...
            secondVariable = ""
            thirdVariable = ""
            selectedOrUnselectedArray = ["[SELECTED]","[UNSELECTED]","\n[REQUIRED]"]
            allAvailableOptions = ["Host-ap with airbase-ng/hostapd","Intercept HTTP links and URLs - Type [2]","Intercept any images - Type [3]","Intercept any unencrypted passwords (not currently working) - Type [4]"]
            clearTerm()
            print(asciiArt)
            ...
            if userConfirmation.lower().startswith("y"):
                ...
                def apToolChoice():
                    apHostMethod = input("[+]What shall I use to host the AP? [%sh%s]ostapd or [%sa%s]irbase-ng?  ~#   " %(col.green_deep,col.endl_deep,col.green_deep,col.endl_deep))
                ...
                channelNumber = input("[?]What channel shall I use?  ~#  ")
                ...
                if thirdVariable != "":
                    selectedOptionThree()
                print("\n[INFO]- AP HOSTING METHOD: %s" %(apHostMethod))
                print("[INFO]- SSID: %s" %(ssid))
                print("[INFO]- CHANNEL: %s" %(channelNumber))
        ...
        #bashRun("echo -n \" // Chipset: \"; echo $(airmon-ng | awk -F ' ' {'print $4,$5,$6,$7'} | grep -v Chipset) ")
            ...
            if chooseCpuOrGpu == "1": #CPU CRACK SECTION
                ...
                while True:
                    ...
                    if hashcatInstallerChoice.lower().startswith("y"):
                        ...
                        clearTerm()
                        continue
                input("\nWhere is the hashcat directory? Please click inside the hashcat-x.x-x folder and press ok. (press enter) ~# ")
                ...
                def hashcatFunction():
                    ...
                    print(menuArt)
                    print("\n[CAP FILE]: %s" %(captureFilePath))
                    print("[HASHCAT PATH]: %s" %(hashcatDirectoryPath))
                    ...
                    def optionDisplayMenu():
                        ...
                        print("\n[CAP FILE]: %s" %(captureFilePath))
                        print("[HASHCAT PATH]: %s" %(hashcatDirectoryPath))
                        print("[HASHCAT-UTILS PATH]: %s" %(hashcatUtilsPath))
                        print("[WORDLIST]: %s" %(mainWordList))
                        print("[ATTACK MODE]: %s" %(hashcatAttackMode))
                        print("[TEMPERATURE RETAIN]: %s" %(gpuTemperatureRetain))
                        print("[TEMPERATURE ABORT]: %s" %(gpuMaximumTemperature))
                    if mainWordList == "N/A":
                        ...
                        if wordlistTypeChoice == "1":
                            ...
                            while True:
                                gpuTemperatureRetain = input("What temperature do you want to retain? (degress celsius) ~# ")
                                gpuMaximumTemperature = input("What should be the cutoff/maximum temperature limit? (degress celsius) ~# ")
        ...
        """)
                            ...
                            while True:
                                ...
                                if gpuTemperatureRetain <= 80 and gpuMaximumTemperature <= 85:
                                    ...
                                    continue
                            optionDisplayMenu()
                            print("[INPUT MASK/CHARSET]: %s" %(hashcatBruteForceMask))
                            if bashReturnValue("ls ~/.airscriptNG/ ") != '0':
                                bashRun("mkdir ~/.airscriptNG/ 2>/dev/null")
                            if input("\nOptions correctly chosen? Start cracking? [y/n] ~# ").lower().startswith("y"):
                                if bashReturnValue("ls ~/.airscriptNG/HANDSHAKEFILE ") != '0':
        ...
        if input("\n\nAdd Airscript-ng commands to system? (append alias to ~/.bash_aliases)? %s[y/n]%s >" %(col.head_deep,col.endl)).lower().startswith("y") and bashReturnValue("cat ~/.bash_aliases 2>/dev/null | grep \"airscript-ng\" ") != '0':
            ...
            while True:
                ...
                if userDirectoryChoice.lower().startswith('y'):
                    ...
                    print("please note though that removing the script will not remove the commands from bash, remove the alias from ~/.bashrc before doing so!")
    ...
    def reaver():
        try:
            ...
            userAcknowledgement = input("[?] %sType 'y' to continue >>%s " %(col.head,col.endl))
    ...
    [info] Developed by Sh3llCod3, Using the reaver tool. 2017-2020. Use with legal caution, disclaimer applies.Thanks for using this program\n""")
                ...
                print("[CIPHER]: %s" %(value_for_csvFileArray[network_select_csv-1][-4]))
                print("[SIGNAL STRENGTH]: %s dBm" %(value_for_csvFileArray[network_select_csv-1][-2]))
                print("[AUTHENTICATION]: %s" %(value_for_csvFileArray[network_select_csv-1][-3]))
    ...
    [info] Developed by Sh3llCod3, Using the Aircrack-ng tool. 2017-2020. Use with legal caution, disclaimer applies.                                                                                  
        """)
                    ...
                    input("[info] 3) Run it for %s5~ seconds%s and press CTRL+C when you see target network. \n\n[?] press enter to continue >>" %(col.green_deep,col.yel_deep))
